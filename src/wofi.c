...
enum sort_order { SORT_ORDER_DEFAULT, SORT_ORDER_ALPHABETICAL };

static uint64_t width, height;
static char *x, *y;
static struct zwlr_layer_shell_v1 *shell = NULL;
static GtkWidget *window, *outer_box, *scroll, *entry, *inner_box,
...
static void get_img_data(char *original, char *str, struct map *mode_map,
                         bool first, char **mode, char **data) {
...
static char *parse_images(WofiPropertyBox *box, const char *text,
                          bool create_widgets) {
  char *ret = strdup("");
  ...
  while (true) {
    if (mode1 == NULL) {
      ...
      if (strcmp(mode1, "img") == 0 && create_widgets) {
        ...
        buf = utils_g_resize_pixbuf(buf, image_size * wofi_get_window_scale(),
                                    GDK_INTERP_BILINEAR);

        GtkWidget *img = gtk_image_new();
        cairo_surface_t *surface = gdk_cairo_surface_create_from_pixbuf(
            buf, wofi_get_window_scale(), gtk_widget_get_window(img));
...
static GtkWidget *create_label(char *mode, char *text, char *search_text,
                               char *action) {
  ...
  if (pre_display_cmd != NULL) {
    ...
    FILE *fp_labeltext;
    char *cmd_labeltext;
    char line[128]; // you'd think this caps the line's length to 128, but it's
                    // just a buffer which due to the nature of fgets() splits
                    // on lines
    size_t size = 0;
    // first, prepare cmd_labeltext to be each entry's actual comamand to run,
    // aka replacing 'cat %s' to be 'cat filename'
    ...
    if (fp_labeltext == NULL) {
      ...
      // lastly, read the output of said command, and put it into the text
      // variable to be used for the label widgets consider using 'printf %.10s
      // as your --pre-display-cmd to limit a string to a determined width. 10
      // here is an example
...
static void activate_item(GtkFlowBox *flow_box, GtkFlowBoxChild *row,
                          gpointer data) {
  (void)flow_box;
  (void)data;
...
static gboolean _insert_widget(gpointer data) {
  ...
  if (node->action_count > 1 && !no_actions) {
    ...
    gtk_list_box_set_activate_on_single_click(GTK_LIST_BOX(exp_box),
                                              single_click);
    ...
    for (size_t count = 1; count < node->action_count; ++count) {
      if (node->builder == NULL) {
        box = create_label(node->mode, node->text[count], node->search_text,
                           node->actions[count]);
      } else {
...
char *wofi_get_dso_path(struct mode *mode) { return mode->dso; }

bool wofi_allow_images(void) { return allow_images; }

bool wofi_allow_markup(void) { return allow_markup; }

uint64_t wofi_get_image_size(void) { return image_size; }

uint64_t wofi_get_window_scale(void) {
  return gdk_window_get_scale_factor(gtk_widget_get_window(window));
}

bool wofi_mod_shift(void) { return mod_shift; }

bool wofi_mod_control(void) { return mod_ctrl; }
...
void wofi_term_run(const char *cmd) {
  ...
  fprintf(
      stderr,
      "No terminal emulator found please set term in config or use --term\n");
  exit(1);
}
...
static void do_resize_surface_after_filter(GtkFlowBoxChild *row,
                                           gboolean filter_return) {
...
static gint do_sort(GtkFlowBoxChild *child1, GtkFlowBoxChild *child2,
                    gpointer data) {
  (void)data;
  gtk_flow_box_get_child_at_index(GTK_FLOW_BOX(inner_box), 0);
  ...
  const gchar *text1 =
      wofi_property_box_get_property(WOFI_PROPERTY_BOX(box1), "filter");
  const gchar *text2 =
      wofi_property_box_get_property(WOFI_PROPERTY_BOX(box2), "filter");
  uint64_t index1 =
      strtol(wofi_property_box_get_property(WOFI_PROPERTY_BOX(box1), "index"),
             NULL, 10);
  uint64_t index2 =
      strtol(wofi_property_box_get_property(WOFI_PROPERTY_BOX(box2), "index"),
             NULL, 10);
  ...
  return sort_for_matching_mode(text1, text2, fallback, matching, filter,
                                insensitive);
}
...
static void do_copy(void) {
  ...
  if (children->data != NULL && gtk_widget_has_focus(children->data)) {
    ...
    if (WOFI_IS_PROPERTY_BOX(widget)) {
      const gchar *action =
          wofi_property_box_get_property(WOFI_PROPERTY_BOX(widget), "action");
      if (action == NULL) {
        return;
      }
...
static bool has_mod(guint state) {
  return (state & GDK_SHIFT_MASK) == GDK_SHIFT_MASK ||
...
static gboolean key_press(GtkWidget *widget, GdkEvent *event, gpointer data) {
  ...
  bool printable =
      strlen(name) == 1 && isprint(name[0]) && !has_mod(event->key.state);
  ...
  if (key_ent != NULL && key_ent->action != NULL) {
    ...
    GList *children =
        gtk_flow_box_get_selected_children(GTK_FLOW_BOX(inner_box));
    if (gtk_widget_has_focus(entry)) {
  ...
  } else if (event->key.keyval == GDK_KEY_Alt_L ||
             event->key.keyval == GDK_KEY_Alt_R) {
  } else {
    key_success = false;
  }
...
static gboolean focus_entry(GtkWidget *widget, GdkEvent *event, gpointer data) {
  ...
  if (widget == entry && dbus != NULL) {
    ...
    GVariant *ret = g_dbus_proxy_call_sync(
        dbus, "SetVisible", g_variant_new("(b)", event->focus_change.in),
...
static void *load_mode(char *_mode, char *name, struct mode *mode_ptr,
                       struct map *props) {
...
static struct mode *add_mode(char *_mode) {
  ...
  void (*init)(struct mode *_mode, struct map *props) =
      load_mode(_mode, _mode, mode_ptr, props);
...
static void get_output_name(void *data, struct zxdg_output_v1 *output,
                            const char *name) {
  (void)output;
...
static void get_output_res(void *data, struct zxdg_output_v1 *output,
...
static void get_output_pos(void *data, struct zxdg_output_v1 *output, int32_t x,
                           int32_t y) {
  (void)output;
  struct output_node *node = data;
  node->x = x;
...
static gboolean do_percent_size(gpointer data) {
  ...
  GdkMonitor *monitor = gdk_display_get_monitor_at_window(
      gdk_display_get_default(), gtk_widget_get_window(window));
...
void wofi_init(struct map *_config) {
  ...
  y = map_get(config, "y");
  bool normal_window =
      strcmp(config_get(config, "normal_window", "false"), "true") == 0;
  char *mode = map_get(config, "mode");
  GtkOrientation orientation = config_get_mnemonic(
      config, "orientation", "vertical", 2, "vertical", "horizontal");
  outer_orientation = config_get_mnemonic(config, "orientation", "vertical", 2,
                                          "horizontal", "vertical");
  GtkAlign halign = config_get_mnemonic(config, "halign", "fill", 4, "fill",
                                        "start", "end", "center");
  content_halign = config_get_mnemonic(config, "content_halign", "fill", 4,
                                       "fill", "start", "end", "center");
  char *default_valign = "start";
  if (outer_orientation == GTK_ORIENTATION_HORIZONTAL) {
    default_valign = "center";
  }
  GtkAlign valign = config_get_mnemonic(config, "valign", default_valign, 4,
                                        "fill", "start", "end", "center");
  char *prompt = config_get(config, "prompt", mode);
  uint64_t filter_rate =
      strtol(config_get(config, "filter_rate", "100"), NULL, 10);
  allow_images =
      strcmp(config_get(config, "allow_images", "false"), "true") == 0;
  allow_markup =
      strcmp(config_get(config, "allow_markup", "false"), "true") == 0;
  ...
  bool hide_scroll =
      strcmp(config_get(config, "hide_scroll", "false"), "true") == 0;
  matching = config_get_mnemonic(config, "matching", "contains", 3, "contains",
                                 "multi-contains", "fuzzy");
  insensitive = strcmp(config_get(config, "insensitive", "false"), "true") == 0;
  parse_search =
      strcmp(config_get(config, "parse_search", "false"), "true") == 0;
  location = config_get_mnemonic(
      config, "location", "center", 18, "center", "top_left", "top",
  ...
  sort_order = config_get_mnemonic(config, "sort_order", "default", 2,
                                   "default", "alphabetical");
  line_wrap = config_get_mnemonic(config, "line_wrap", "off", 4, "off", "word",
                                  "char", "word_char") -
              1;
  bool global_coords =
      strcmp(config_get(config, "global_coords", "false"), "true") == 0;
  bool hide_search =
      strcmp(config_get(config, "hide_search", "false"), "true") == 0;
  char *search = map_get(config, "search");
  dynamic_lines =
      strcmp(config_get(config, "dynamic_lines", "false"), "true") == 0;
  ...
  single_click =
      strcmp(config_get(config, "single_click", "false"), "true") == 0;
  ...
  for (int i = 0; i < 2; i++) {
    ...
    key_default = "Down";
    char *key_down =
        (i == 0) ? key_default : config_get(config, "key_down", key_default);
    key_default = "Left";
    char *key_left =
        (i == 0) ? key_default : config_get(config, "key_left", key_default);
    key_default = "Right";
    char *key_right =
        (i == 0) ? key_default : config_get(config, "key_right", key_default);
    key_default = "Tab";
    char *key_forward =
        (i == 0) ? key_default : config_get(config, "key_forward", key_default);
    key_default = "Shift-ISO_Left_Tab";
    char *key_backward = (i == 0)
                             ? key_default
                             : config_get(config, "key_backward", key_default);
    key_default = "Return";
    char *key_submit =
        (i == 0) ? key_default : config_get(config, "key_submit", key_default);
    key_default = "Escape";
    char *key_exit =
        (i == 0) ? key_default : config_get(config, "key_exit", key_default);
    key_default = "Page_Up";
    char *key_pgup =
        (i == 0) ? key_default : config_get(config, "key_pgup", key_default);
    key_default = "Page_Down";
    char *key_pgdn =
        (i == 0) ? key_default : config_get(config, "key_pgdn", key_default);
    key_default = "";
    char *key_expand =
        (i == 0) ? key_default : config_get(config, "key_expand", key_default);
    key_default = "";
    char *key_hide_search =
        (i == 0) ? key_default
                 : config_get(config, "key_hide_search", key_default);
    key_default = "Ctrl-c";
    char *key_copy =
        (i == 0) ? key_default : config_get(config, "key_copy", key_default);

    char *keys_custom[CUSTOM_KEY_NUMBER];
    key_default = "";
    for (uint8_t kc_index = 0;
         kc_index < (sizeof(keys_custom) / sizeof(char *)); kc_index++) {
      char config_entry[15];
      sprintf(config_entry, "key_custom_%d", kc_index);
      keys_custom[kc_index] =
          (i == 0) ? key_default
    ...
    add_key_entry(key_submit,
                  NULL); // submit is a special case, when a NULL action is
                         // encountered submit is used instead
...
#define CUSTOM_KEY_FUNC(index) do_custom_key_##index
  ...
  if (!normal_window) {
    ...
    struct wl_registry_listener listener = {.global = add_interface,
                                            .global_remove = nop};
    wl_registry_add_listener(registry, &listener, NULL);
    wl_display_roundtrip(wl);

    if (shell == NULL) {
      fprintf(stderr, "Compositor does not support wlr_layer_shell protocol, "
                      "switching to normal window mode\n");
      normal_window = true;
      goto normal_win;
    }
    ...
    wl_list_for_each(node, &outputs, link) {
      ...
      struct zxdg_output_v1_listener *xdg_listener =
          malloc(sizeof(struct zxdg_output_v1_listener));
    ...
    if (global_coords) {
      wl_list_for_each(node, &outputs, link) {
        if (ix >= node->x && ix <= node->width + node->x && iy >= node->y &&
            iy <= node->height + node->y) {
          output = node->output;
          ix -= node->x;
          iy -= node->y;
          break;
    ...
    wlr_surface = zwlr_layer_shell_v1_get_layer_surface(
        shell, wl_surface, output, wlr_layer, "wofi");
...
normal_win:
  ...
  gtk_flow_box_set_selection_mode(GTK_FLOW_BOX(inner_box),
  ...
  gtk_flow_box_set_activate_on_single_click(GTK_FLOW_BOX(inner_box),
                                            single_click);
  ...
  g_signal_connect(inner_box, "child-activated", G_CALLBACK(activate_item),
                   NULL);
  g_signal_connect(inner_box, "selected-children-changed",
                   G_CALLBACK(select_item), NULL);
  ...
  dbus = g_dbus_proxy_new_for_bus_sync(
      G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, NULL, "sm.puri.OSK0",
      "/sm/puri/OSK0", "sm.puri.OSK0", NULL, NULL);
