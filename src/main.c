...
static void print_usage(char **argv) {
  ...
  printf("--show\t\t\t-S\tSpecifies the mode to run in. A list can be found in "
         "wofi(7)\n");
  ...
  printf("--exec-search\t\t-e\tMakes enter always use the search contents not "
         "the first result\n");
  ...
  printf("--parse-search\t\t-q\tParses the search text removing image escapes "
         "and pango\n");
  printf("--version\t\t-v\tPrints the version and then exits\n");
  printf("--location\t\t-l\tSets the location\n");
  printf("--no-actions\t\t-a\tDisables multiple actions for modes that support "
         "it\n");
  ...
  printf("--pre-display-cmd\t-r\tRuns command for the displayed entries, "
         "without changing the output. %%s for the real string\n");
  exit(0);
}
...
void wofi_load_css(bool nyan) {
  if (access(stylesheet, R_OK) == 0) {
    ...
    if (count > 99) {
      fprintf(stderr, "Woah there that's a lot of colors. Try having no more "
                      "than 100, thanks\n");
      exit(1);
    }
    struct node *node;
    wl_list_for_each(node, &lines, link) {
      ...
      while (replace != NULL) {
        ...
        b[2] = 0;
        char rgb[14];
        snprintf(rgb, 14, "%ld, %ld, %ld", strtol(r, NULL, 16),
...
int main(int argc, char **argv) {
  ...
  if (password_char == NULL ||
      (password_char != NULL && strcmp(password_char, "false") != 0)) {
    if (password_char == NULL) {
      password_char = "*";
    }
