...
static char *get_search_text(char *file) {
  ...
  char *ret = utils_concat(7, name, file, exec == NULL ? "" : exec,
                           description == NULL ? "" : description,
                           categories == NULL ? "" : categories, keywords_str,
                           generic_name == NULL ? "" : generic_name);
  free(keywords_str);
  return ret;
}

static bool populate_widget(char *file, char *action,
  ...
  if (wofi_allow_images()) {
    ...
    if (G_IS_FILE_ICON(icon)) {
      ...
      if (icon != NULL) {
        ...
        info = gtk_icon_theme_choose_icon_for_scale(
            theme, (const gchar **)icon_names, wofi_get_image_size(),
            wofi_get_window_scale(), 0);
      }
      if (info == NULL) {
        info = gtk_icon_theme_lookup_icon_for_scale(
            theme, "application-x-executable", wofi_get_image_size(),
            wofi_get_window_scale(), 0);
      }
    ...
    pixbuf = utils_g_resize_pixbuf(
        pixbuf, wofi_get_image_size() * wofi_get_window_scale(),
        GDK_INTERP_BILINEAR);
...
static struct widget_builder *populate_actions(char *file, size_t *text_count) {
  ...
  for (size_t count = 1; count < *text_count; ++count) {
    populate_widget(file, (gchar *)action_names[count - 1],
...
void wofi_drun_init(struct mode *this, struct map *config) {
  ...
  print_command =
      strcmp(config_get(config, "print_command", "false"), "true") == 0;
  display_generic =
      strcmp(config_get(config, "display_generic", "false"), "true") == 0;
  disable_prime =
      strcmp(config_get(config, "disable_prime", "false"), "true") == 0;
...
void wofi_drun_exec(const gchar *cmd) {
  ...
  if (G_IS_DESKTOP_APP_INFO(info)) {
    ...
    if (print_command) {
      ...
      set_dri_prime(info);
      if (uses_dbus(info)) {
        g_app_info_launch_uris_async(G_APP_INFO(info), NULL, NULL, NULL,
                                     launch_done, (gchar *)cmd);
      } else {
    ...
    *space = 0;
    ...
    if (print_command) {
      ...
      free(cmd);
      fprintf(stderr,
              "Printing the command line for an action is not supported\n");
    } else {
      set_dri_prime(info);
...
const char **wofi_drun_get_arg_names(void) { return arg_names; }
