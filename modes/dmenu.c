...
void wofi_dmenu_init(struct mode *this, struct map *config) {
  mode = this;
  parse_action =
      strcmp(config_get(config, "parse_action", "false"), "true") == 0;
  separator = config_get(config, "separator", "\n");
  print_line_num =
      strcmp(config_get(config, "print_line_num", "false"), "true") == 0;
  ...
  if (!print_line_num) {
    ...
    wl_list_for_each_safe(node, tmp, cache, link) {
      if (map_contains(entry_map, node->line)) {
        ...
        widget->widget =
            wofi_create_widget(mode, &node->line, node->line, &node->line, 1);
  ...
  wl_list_for_each_reverse_safe(node, tmp, &entries, link) {
    if (!map_contains(cached, node->line)) {
      ...
      struct node *widget = malloc(sizeof(struct node));
      widget->widget =
          wofi_create_widget(mode, &node->line, node->line, &action, 1);
      wl_list_insert(&widgets, &widget->link);
      free(action);
    }
    free(node->line);
...
const char **wofi_dmenu_get_arg_names(void) { return arg_names; }

size_t wofi_dmenu_get_arg_count(void) {
  return sizeof(arg_names) / sizeof(char *);
}

bool wofi_dmenu_no_entry(void) { return true; }
